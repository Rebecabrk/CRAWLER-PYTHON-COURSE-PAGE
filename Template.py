...
def write_function_name(statement, exercise_number, file):
    if re.search(r'\bclass\b', statement):
        ...
        class_names = [n for n in class_names if n != 'Write' and n != 'Create' and n !=
                       'Python' and n != 'None' and n != 'Design' and n != 'NxM' and len(n) > 1]
        ...
        function_name = re.search(
            r'\bThe\s(\w+)\sfunction\b', statement).group().split()[1]
...
main = soup.find('main')
filtered_a_tags = [a for a in main.find_all(
    'a') if a.find_parent('h1') is None]
        ...
        for index, ol_tag in enumerate(main.find_all('ol')):
            if ol_tag.find_next_sibling('pre') is not None:
                parse_ol_tag(ol_tag, index, f,
                             pre_tag=ol_tag.find_next_sibling('pre'))
            else:
                parse_ol_tag(ol_tag, index, f)
